...
def train_ddp(
    ...
    if model_filename:
        ...
        # All ranks load from the same file so model state is consistent before DDP sync
    ...
    for epoch in range(initial_epoch, config.num_epochs):
        # Required so DistributedSampler shuffles differently each epoch
        ...
        for batch in batch_iterator:
            ...
            # Call encode/decode/project on model.module directly since Translator
            # exposes them as separate methods rather than a single forward().
            # DDP gradient synchronization still fires correctly during loss.backward()
            # via the per-parameter all-reduce hooks registered at DDP init time.
